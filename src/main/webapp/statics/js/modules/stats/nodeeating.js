$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '', name: 'personNum', index: 'person_num', width: 80 }, 			
			{ label: '', name: 'eatingDate', index: 'eating_date', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.nodeEating.id == null ? "nodeeating/save" : "nodeeating/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.nodeEating),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "nodeeating/info/"+id, function(r){
                vm.nodeEating = r.nodeEating;
            });
		},
