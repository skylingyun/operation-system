$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '', name: 'otherDate', index: 'other_date', width: 80 }, 			
			...
			{ label: '发票类型', name: 'invoicetype', index: 'invoicetype', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.nodeOther.id == null ? "nodeother/save" : "nodeother/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.nodeOther),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
			$.get(baseURL + "nodeother/info/"+id, function(r){
                vm.nodeOther = r.nodeOther;
            });
		},
